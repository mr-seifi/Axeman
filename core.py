    ...
    try:
        ...
        for entry in result_info['entries']:
            ...
            for domain in cert_data['leaf_cert']['all_domains']:
                if domain.split('.')[:-2]:
                    print('[+] domain:', domain)
                    client.incr('.'.join(domain.split('.')[:-2]))
            ...
            #         str(cert_data['leaf_cert']['not_after']), # ['crlDistributionPoints'].split('\n')[1].strip().replace('URI:', '').split('/')[2].split('.')[0:-2]
            #     ]) + "\n"
            # )
